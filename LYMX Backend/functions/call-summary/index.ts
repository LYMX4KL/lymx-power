...
//   * Events: meeting.ended, transcript.ready, recording.ready
...
//   2. transcript.ready fires → fetch the transcript JSON from Daily, save the
//      raw text to bookings.transcript.
...
serve(async (req) => {
    ...
    if (eventType === "meeting.ended" || eventType === "room.meeting.ended") {
        ...
        return json({ ok: true, action: "marked_completed", booking_id: booking.id });
    }

    if (eventType === "transcript.ready" || eventType === "recording.ready") {
